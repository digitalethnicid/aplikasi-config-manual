...
document
  ...
  .addEventListener("click", convertToJson, false);
document
  .getElementById("downloadButton")
  .addEventListener("click", downloadJson, false);
document
  .getElementById("searchInput")
  .addEventListener("input", filterResults, false);
document
  .getElementById("clearData")
  .addEventListener("click", clearIndexedDB, false);
...
function saveToIndexedDB(newData) {
  ...
  request.onsuccess = function (event) {
    ...
    saveRequest.onerror = function (event) {
      console.error(
        "Error saving combined data to IndexedDB:",
...
function convertToJson() {
  const columnSelect = document.getElementById("columnSelect");
  selectedColumns = Array.from(columnSelect.selectedOptions).map(
    (option) => option.value
  );
...
function filterResults() {
  ...
  getFromIndexedDB((data) => {
    ...
    limitedData.forEach((item) => {
      ...
      // Iterasi pada properti objek `item`
      Object.keys(item).forEach((field) => {
        const input = document.createElement("input");
        input.type = "text";

        input.value = item[field]; // Mengambil nilai dari properti
